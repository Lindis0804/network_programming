...
inline int send_stream(int client, char *buf, int buf_size, int flag,
                       int BUFF_SIZE) {
  int l;
  char msg[BUFF_SIZE];
  int ret;
  ...
  for (; i < l; i++) {
    ...
    ret = send(client, msg, BUFF_SIZE, flag);
    sret += ret;
  }
  return sret;
}
inline int send_stream_msg(int client, char *buf, int buf_size, int flag,
                           int BUFF_SIZE) {
  int sret = send_stream(client, buf, buf_size, flag, BUFF_SIZE);
  int ret = send(client, "\0", 1, flag);
  return sret;
}
inline int recv_stream(int client, char *buf, int buf_size, int flag,
                       int STREAM_SIZE) {
  int ret;
  int sret = 0;
  char stream[256];
  strcpy(buf, "");
  while (1) {
    ret = recv(client, stream, STREAM_SIZE, flag);
    stream[ret] = 0;
    sret += ret;
    strcat(buf, stream);
    if (strlen(stream) <= 0)
...
inline int send_file(int client, char *filename, int BUFF_SIZE) {
  ...
  while (fgets(str, sizeof(str), ptr)) {
    ret = send_stream(client, str, strlen(str), 0, BUFF_SIZE);
    sret += ret;
  }
